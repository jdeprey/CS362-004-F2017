...
int main (int argc, char** argv) {
    ...
    for (i = 0; i < NUM_TESTS; i++) {
        ...
        result = cardEffect(adventurer, -1, -1, -1, state, 0, 0);
        if (result != 0) {
            failCount++;
        }

        // Check that hand count increased by 1 (1 discard, 2 treasures added)
        if (state->handCount[currentPlayer] != original->handCount[currentPlayer] + 1) {
            failCount++;
        }
        // Check that deck has been decremented by at least 2 for the treasure cards (and others) drawn
        if (state->deckCount[currentPlayer] <= original->deckCount[currentPlayer] - 2) {
            failCount++;
        }
        free(state);
        free(original);
